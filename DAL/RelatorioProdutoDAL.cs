...
namespace GVC.DAL
{
    public class RelatorioProdutoDAL : RepositoryBase
    {
        // ============================
        ...
        public List<RelatorioProdutoEstoqueDTO> ObterProdutosEstoque()
        {
            const string sql = @"
                        ...
                        FROM Produtos
                        WHERE Status = 'Disponível' AND EmpresaID = @EmpresaID
                        ORDER BY NomeProduto";

            return Connection
                .Query<RelatorioProdutoEstoqueDTO>(sql)
                .ToList();
        }
            ...
            DateTime? dataFinal)
        {
            var sql = @"
                    ...
                    WHERE v.StatusVenda <> 'Cancelada' AND v.EmpresaID = @EmpresaID ";
            ...
            return Connection
                .Query<RelatorioLucroProdutoDTO>(sql, new
                {
